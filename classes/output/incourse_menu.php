...
class incourse_menu implements \renderable, \templatable {
    ...
    /**
     * The if every video in the resource has been watched.
     * @var int
     */
    public $watchedall;
    ...
    */
     ...
     * @return void
     */
    public function __construct(string $type, string $externalref, array $response = [], int $firstload, int $watchedall = 0) {
        $this->type = $type;
        switch ($this->type) {
            case 'collections':
    ...
    public function export_array_for_template(renderer_base $output): array {
        ...
        $response['type'] = $this->type;
        $response['firstload'] = $this->firstload;
